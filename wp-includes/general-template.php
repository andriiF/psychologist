    ...
    if ('' != $search_form_template) {
        ...
        if ('html5' == $format) {
            $form = '<form role="search" method="get" class="search-form" action="' . esc_url(home_url('/')) . '">
 			...
 			<input type="search" class="search-field" placeholder="' . esc_attr_x('Search Psychics / Services', 'placeholder') . '" value="' . get_search_query() . '" name="s" />
                        <a href="#"  class="icon">
...
function get_search_small_form($echo = true) {
    /**
     * Fires before the search form is retrieved, at the start of get_search_form().
     *
     * @since 2.7.0 as 'get_search_form' action.
    ...
    switch ($show) {
        ...
        case 'siteurl' : // DEPRECATED
            _deprecated_argument(__FUNCTION__, '2.2.0', sprintf(
                            ...
                            __('The %1$s option is deprecated for the family of %2$s functions. Use the %3$s option instead.'), '<code>' . $show . '</code>', '<code>bloginfo()</code>', '<code>url</code>'
            ));
        case 'url' :
            $output = home_url();
            break;
        ...
        case 'text_direction':
            _deprecated_argument(__FUNCTION__, '2.2.0', sprintf(
                            ...
                            __('The %1$s option is deprecated for the family of %2$s functions. Use the %3$s function instead.'), '<code>' . $show . '</code>', '<code>bloginfo()</code>', '<code>is_rtl()</code>'
            ));
    ...
    for ($day = 1; $day <= $daysinmonth; ++$day) {
        ...
        if (in_array($day, $daywithpost)) {
            ...
            $calendar_output .= sprintf(
                    '<a href="%s" aria-label="%s">%s</a>', get_day_link($thisyear, $thismonth, $day), esc_attr($label), $day
            );
        } else {
            $calendar_output .= $day;
        }
...
function register_admin_color_schemes() {
    ...
    wp_admin_css_color('fresh', _x('Default', 'admin color scheme'), false, array('#222', '#333', '#0073aa', '#00a0d2'), array('base' => '#82878c', 'focus' => '#00a0d2', 'current' => '#fff')
    );
